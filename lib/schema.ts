...
// take the raw-attribute & maps it to attributeSchema, then refine() to check validity
export const attributeSchema = z.preprocess(
  (attr: any) => ({
    ...
    attributeType: String(
      attr?.attributeType || attr?.data?.attributeType || "simple",
    )
      .toLowerCase()
      .trim(),
...
export const entitySchema = z
  .object({
    ...
    data: z.object({
      ...
      primaryKey: z
        .string()
        .min(1, { message: "Table is missing a Primary Key" }),
    }),
    attributes: z
      .array(attributeSchema)
      .min(1, { message: "Table must have at least one attribute" }),
  })
  .refine(
    (entity) => {
